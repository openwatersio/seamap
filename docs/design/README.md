...
These documents record how this chart decides what to draw and how to draw it. They capture the posture, rules, and deliberate departures from chart standards. Each departure includes its rationale so that a future audit can distinguish a decision from an accident.
...
**This is not a type-approved ECDIS presentation.** It is a chart for planning and situational awareness. That freedom makes the design work possible. It may be spent on presentation, never on meaning.

This chart presents crowd-sourced OpenStreetMap seamark data in the visual language of paper nautical charts. IHO standards are the reference: S-4 and INT1 for symbol meaning and convention; S-52 and S-101 for hazard logic and portrayal semantics. The standards supply semantic rigor, hazard hierarchy, and completeness. The presentation adds modern cartographic craft: restrained color, real typography, purposeful hierarchy, and density that responds to zoom.

Where the presentation differs from a standard, the chart is either still converging or the departure and its safety rationale are recorded below.
...
Glanceability target: on a 9″ daylight display at arm's length, the primary hazard question — _am I standing into danger?_ — can be answered in under two seconds.

## Invariants

Settle every symbol decision against these invariants. When principles conflict, the invariant wins.
...
4. **Presence beats completeness.** A base symbol without its topmark communicates more than an absent mark. At small scales the chart promises class and presence — _a cardinal is here_ — and restores full identity as soon as the screen can resolve it. It never substitutes an abstract stand-in: a dot has no remaining channel for meaning, while a shrunken body still identifies the class.
...
These are presentation choices and therefore the proper design surface:
...
6. **The pipeline is part of the design surface.** When MapLibre cannot compute something, the pipeline precomputes it: sector geometry, `cell_rank`, `near_shore`, and seabed samples. The priority is safety, then beauty, then technical convenience. Define the right presentation first and find a way to render it. Bend the design only when neither preprocessing nor the style can express it, and preserve the intent when doing so.
7. **Never trade meaning for beauty.**

## Deliberate departures

These are intentional departures from the standards. Anything else is either compliant, still converging, or a bug.
...
- **Class-based SCAMIN is replaced with density-based instance ranking** (`cell_rank`). The S-57 UOC notes that class steps "take no direct account of the relative importance of individual occurrences," while its optional rules rank instances manually. The tile pipeline automates that instance-level refinement.
...
- **`icon-overlap: "always"` on marks** — MapLibre silently dropping an aid to navigation is strictly worse than overlap. Density is governed by budgets instead; see [Visibility](visibility.md).
...
Evaluate changes against these gates:
...
The concrete regression scenes — places that have each caught a wrong rule — live in [Visibility](visibility.md).
