 ...
 *
 *  - contentEtag: a validator derived from tile bytes, so a release that leaves
 *    a tile unchanged leaves its validator unchanged — clients revalidate to
 *    bodyless 304s instead of re-downloading. The version never enters the tag.
 *
...
export class CachedSource implements Source {
  inner: Source;
  // The version inside keyBase makes every key immutable: a release switches
  // namespaces and superseded entries LRU out on their own. Keep the host on
  // the served zone (tiles.openwaters.io) so a manual dashboard
