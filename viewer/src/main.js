...
// vector depth bands for the raster DEM color-relief. ?basemap=false drops
// the base-map mariner preference, leaving only the chart and its topography.
const params = new URLSearchParams(location.search);
const tiles = params.get("tiles") || undefined;
const depthHillshade = params.has("hillshade");
const basemap = params.get("basemap") === "false" ? false : undefined;
...
const map = new maplibregl.Map({
  ...
  zoom: 13.4,
  container: "map",
  style: await style({ tiles, depthHillshade, shading, basemap }),
  dragRotate: false,
  touchPitch: false,
  maxPitch: 0,
  attributionControl: {
