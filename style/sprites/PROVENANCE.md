...
The `poi-*` sprites are not part of the set above and carry none of its
obligations. They are composed at build time by `bin/poi-badges` from
[Maki](https://github.com/mapbox/maki) and
[Temaki](https://github.com/rapideditor/temaki), both **CC0-1.0**, consumed as
the `@iconify-json/maki` and `@iconify-json/temaki` npm packages. The badge
around each glyph — white halo, magenta disc — is trivial geometry generated by
that script.

The borrowed sheet has no facility symbols at all: nothing for a slipway, fuel
dock, pump-out or freshwater tap, while
`seamark:small_craft_facility:category=slipway` alone is the most common
facility tag in OSM. Rather than drawing them, `sprites/poi-icons.json` maps a
